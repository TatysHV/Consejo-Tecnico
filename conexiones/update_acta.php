...
include "conexion.php";
  ...
  if($fichero == "" && $minuta == ""){ // 0 0
    // Si el input de archivo está vacío, no se modificará el archivo en la base de datos
  ...
  elseif($fichero != "" && $minuta == ""){ // 1 0
    // Si se ha agregado un nuevo documento para sustituir el anterior.
      $query = mysqli_query($con, "UPDATE actas SET nombre = '$nombre', tipo_sesion = '$tipo', fecha_sesion = '$fecha',
    ...
    /*+++++++++++++++++++++++++++++++++++++++
      ...
      foreach ($_FILES['archivos']['name'] as $i => $name) { //Evita el uso del array y garantiza su ejecución
        //mientras haya un uno o más archivos en el array y obtiene el nombre del archivo en la posición $i del array.
          ...
          //  echo '<script> /consejo_tecnico/sesiones.php"</script>';
     		   }
     	   }
       }
    }

    elseif($fichero != "" && $minuta != ""){ // 1 1
      $query = mysqli_query($con, "UPDATE actas SET nombre = '$nombre', tipo_sesion = '$tipo', fecha_sesion = '$fecha',
      numero_sesion ='$numero', pdf = '$fichero', minuta='$minuta' WHERE id = '$id_acta'");
      ...
      foreach ($_FILES['archivos']['name'] as $i => $name) { //Evita el uso del array y garantiza su ejecución
       ...
       foreach ($_FILES['minuta']['name'] as $i => $name) { //Evita el uso del array y garantiza su ejecución
         //mientras haya un uno o más archivos en el array y obtiene el nombre del archivo en la posición $i del array.
         if (strlen($_FILES['minuta']['name'][$i]) > 1) { //Garantiza que la cat de caracteres del nombre sea mayor a 1 (No es esencial).
           if (move_uploaded_file($_FILES['minuta']['tmp_name'][$i], $target_path.$name)) {
             //Copia el archivo a la dirección específica de la concatenación: ../archivos/ordendia/nombre.
             echo "El archivo <b>". basename($_FILES['minuta']['name'][$i])." </b>ha sido subido.</br>";
           //  echo '<script> window.location="2016/consejo_tecnico/sesiones.php"</script>';
            }
          }
        }
    ...
    elseif($fichero == "" && $minuta != ""){ // 0 1
      $query = mysqli_query($con, "UPDATE actas SET nombre = '$nombre', tipo_sesion = '$tipo', fecha_sesion = '$fecha',
      numero_sesion ='$numero', minuta = '$minuta' WHERE id = '$id_acta'");
