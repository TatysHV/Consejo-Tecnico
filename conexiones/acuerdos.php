...
function add_acuerdo()
{
  ...
  /*if($url_acta!=""){*/
    ...
    foreach ($_FILES['oficio']['name'] as $i => $name) {
//Evita el uso del array y garantiza su ejecución
      ...
      //----------- Subir la info de cada archivo a la base de datos------------
        ...
        if (strlen($_FILES['oficio']['name'][$i]) > 1) {
//Garantiza que la cant de caracteres del nombre sea mayor a 1 (No es esencial).
            if (move_uploaded_file($_FILES['oficio']['tmp_name'][$i], $target_path.$name)) {
            } else {
                echo "Error, no se han subido los archivos";
            }
        }
    }
...
//Evita el uso del array y garantiza su ejecución
      ...
      //----------- Subir la info de cada archivo a la base de datos------------
        ...
        if (strlen($_FILES['oficio_word']['name'][$i]) > 1) {
//Garantiza que la cant de caracteres del nombre sea mayor a 1 (No es esencial).
    ...
    foreach ($_FILES['acta_admin']['name'] as $i => $name) {
//Evita el uso del array y garantiza su ejecución
      //mientras haya un uno o más archivos en el array y obtiene el nombre del archivo en la posición $i del array.
      ...
      //----------- Subir la info de cada archivo a la base de datos------------
        ...
        if (strlen($_FILES['acta_admin']['name'][$i]) > 1) {
//Garantiza que la cant de caracteres del nombre sea mayor a 1 (No es esencial).
...
function add_acuerdo_file($id_acuerdo)
{
  ...
  //----------Subir cada uno de los archivos a la carpeta del servidor
    foreach ($_FILES['acuerdo_files']['name'] as $i => $name) {
//Evita el uso del array y garantiza su ejecución
      //mientras haya un uno o más archivos en el array y obtiene el nombre del archivo en la posición $i del array.
      ...
      //---------------- Sube los archivos al servidor ---------------------------
        if (strlen($_FILES['acuerdo_files']['name'][$i]) > 1) {
  //Garantiza que la cant de caracteres del nombre sea mayor a 1 (No es esencial).
...
function edit_acuerdo()
{
    ...
    if ($fword != "") {
//----------Subir cada uno de los archivos a la carpeta del servidor
        foreach ($_FILES['oficio_word']['name'] as $i => $name) {
  //Evita el uso del array y garantiza su ejecución
          //mientras haya un uno o más archivos en el array y obtiene el nombre del archivo en la posición $i del array.
